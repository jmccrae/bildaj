...
object JsonLDSchema {
  ...
  def fromJson(elem : JsonElement) : JsonLDSchema = elem match {
    case JsonString(urlStr) => {
      try {
        val url = new URL(urlStr)
        val jsonParser = new JsonFactory().createJsonParser(url.openStream())
        val obj = JsonUtils.read(jsonParser)
        return fromJson(obj)
      } catch {
        ...
        case x : JsonReadingException => throw new JsonLDException("Not a valid JSON object at %s" format urlStr)
      }
    }
    case JsonArray(elems) => elems match {
      case Nil => throw new JsonLDException("Empty array!")
      case _ =>  elems.map(fromJson(_)).reduce((x,y) => x ++ y)
    }
